#include "../include/fides/minimize.hpp"

#include "../include/fides/trust_region.hpp"
  ...
  , starttime_(std::chrono::milliseconds::zero())
{
    ...
    grad_min_ = grad_;
    f_data_ = f_data;
    spdlog::set_level(spdlog::level::err);//(spdlog::level::debug);
...
Optimizer::minimize(const DynamicVector<double>& x0)
{
    ...
    std::tie(fval_, grad_, hess_) = cost_fun_(x_, x_, f_data_,false);
    ...
    while (check_continue()) {
        ...
        std::tie(fval_new, grad_new, hess_new) = cost_fun_(x_, x_new, f_data_,false);
        ...
        if (accepted) {
            cost_fun_(x_, x_new, f_data_,true);
