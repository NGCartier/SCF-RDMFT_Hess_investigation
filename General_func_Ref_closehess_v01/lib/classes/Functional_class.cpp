...
MatrixXd Functional::dv_J_n(RDM1* gamma, int i) const {
    ...
    MatrixXd no = gamma->no(); 
    MatrixXd delta = MatrixXd::Zero(l,l); delta(i,i) = 1.;
...
double Functional::ddW_J_n(RDM1* gamma, int i, int j) const {
    ...
    MatrixXd no = gamma->no();
    MatrixXd delta_i = MatrixXd::Zero(l,l); delta_i(i,i) = 1;
    MatrixXd delta_j = MatrixXd::Zero(l,l); delta_j(j,j) = 1;
...
VectorXd Functional::dE_Hxc_aux(RDM1* gamma, bool only_n, bool only_no){
    ...
    if (not only_no){
        ...
        dE2.segment(0,l) = vJ.diagonal() - dn_K.cwiseProduct(vK.diagonal());
    }
    if (not only_n){
        MatrixXd U = Givens(MatrixXd::Identity(l,l),&gamma->l_theta);
...
MatrixXd Functional::ddE_approx(RDM1* gamma, MatrixXd* H_approx, bool only_n, bool only_no) const{
    ...
    MatrixXd ddE = J.transpose()*(*H_approx)*J;
...
MatrixXd ddE1(RDM1* gamma,bool only_n, bool only_no){
    ...
    if (not only_no){
        VectorXd dE1 = H1.diagonal();
        for (int i=0;i<l;i++){
            for (int j=0;j<=i;j++){
                ddE1(i,j) = gamma->ddn(i,j).dot(dE1);
                ddE1(j,i) = ddE1(i,j);
            }
        }
    }
...
MatrixXd Functional::ddE_Hxc(RDM1* gamma, bool only_n, bool only_no) const{
    ...
    if (not only_no){
        ...
        for(int i=0;i<l;i++){
            for(int j=0;j<=i;j++){
                ddE2(i,j) = gamma->ddn(i,j).dot(vJ.diagonal()) - gamma->ddn(i,j).dot(dnvK) ;
                ddE2(j,i) = ddE2(i,j);
            }
        }
    }
...
MatrixXd Functional::ddE_Hxc_aux(RDM1* gamma, bool only_n, bool only_no) const{
    ...
    if (not only_no){ 
        //Tensor<double,4> IJ = ddI_J(gamma);  //Version with Tensors doesn't work
        //Tensor<double,4> IK = ddI_K(gamma);
        vK = v_K(gamma);
        for (int i=0;i<l;i++){
            for (int j=0;j<=i;j++){
                //ddE2(i,j) = IJ(i,i,j,j)   - IK(i,i,j,j)*dn_Kn_(gamma,i)*dn_Kn_(gamma,j)  - (i==j)*vK(i,i)*ddn_Kn_(gamma,i); 
                ddE2(i,j) = ddW_J_n(gamma,i,j) - ddW_K_n(gamma,i,j) - (i==j)*vK(i,i)*ddn_Kn_(gamma,i);
                ddE2(j,i) = ddE2(i,j);
            }
        }
    }
    ...
    if( not only_n && not only_no){
        ...
        for (int i=0;i<l;i++){
            for (int j=0;j<l;j++){
                int id = (i+1)*l+j;
                ddE2(id,j) += 2.*vJU(i,j) - 2.*vKU(i,j)*dn_Kn_(gamma,j);
                for (int p=0;p<l;p++){
